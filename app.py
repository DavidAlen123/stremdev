...
sku_summary = df_filtered_non_cancelled.groupby('SKU').agg({'Total Price': 'sum', 'Quantity': 'sum', 'Title': 'first', 'Vendor': 'first','Created At':'first','Cancelled At':'first'}).reset_index()
...
vendor_wise_sale_total = df_filtered_non_cancelled.groupby('Vendor')['Total Price'].sum().reset_index(name='Total Price')
...
vendor_summary = df_filtered_non_cancelled.groupby('Vendor').agg({'Total Price': 'sum', 'Quantity': 'sum', 'Title': 'first', 'SKU': 'first', 'Created At': 'first', 'Cancelled At': 'first'}).reset_index()
